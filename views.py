                ...
                # if (user.password_hash=="")==True:
                    try:
                        ...
                        if res!=False:
                            ...
                            t=threading.Timer(600,removeOTP,args=[str(user.email)])
                            t.start()
                ...
                # if (user_mail.password_hash=="")==True:
                    ...
                    try:
                        if res is not False:
                            ...
                            t=threading.Timer(600,removeOTP,args=[str(user_mail.email)])
                            t.start()
                            return make_response(jsonify({'res':'success'}))
        ...
        if user is not None and user.password_hash==req_json['pwd']:
            ...
            # user.email
                    if user.email_confirmed and user.account_confirmed:
                        ...
                        return make_response(jsonify({'res':'true','da_t_e':str(now.year),'userId':str(session['user']),'user':str(user.username)}))
                        ...
                        return make_response(jsonify({'res':'accountUnconfirmed','da_t_e':str(now.year),'userId':str(session['user']),'user':str(user.username)}))
                    ...
                    if user_mail.email_confirmed and user_mail.account_confirmed:
                        ...
                        return make_response(jsonify({'res':'true','da_t_e':str(now.year),'userId':session['user'],'user':user_mail.username}))
                        #  return redirect(request.args.get('next') or url_for('index'))
                        ...
                        return make_response(jsonify({'res':"accountUnconfirmed",'da_t_e':str(now.year),'userId':session['user'],'user':user_mail.username}))
